...
let iotHubClient = null;
if (DEVICE_CONNECTION_STRING) {
    try {
        iotHubClient = azure_iot_device_1.Client.fromConnectionString(DEVICE_CONNECTION_STRING, azure_iot_device_mqtt_1.Mqtt);
...
async function sendTelemetry() {
    ...
    if (iotHubClient) {
        try {
            const message = new azure_iot_device_1.Message(JSON.stringify(payload));
